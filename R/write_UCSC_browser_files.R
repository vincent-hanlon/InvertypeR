...
#'
#' @export
write_UCSC_browser_files <- function(
    inversions = NULL,
    WW_reads = NULL,
    WC_reads = NULL,
    confidence = 0.95,
    ...
    if (is.data.frame(region) && nrow(region) > 0) {
        ...
        region <- GenomicRanges::reduce(widen(granges = GenomicRanges::makeGRangesFromDataFrame(region,
            ignore.strand = TRUE, seqnames.field = names(region)[1],
            start.field = names(region)[2], end.field = names(region)[3]
        ), seqlengths = GenomeInfoDb::seqlengths(WW_reads), distance = 5e06), min.gapwidth = 1000)
        ...
        utils::write.table(paste0("track name=", prefix, "inversions itemRgb=On"),
            file = savefile.invs.gz, row.names = FALSE, col.names = FALSE,
