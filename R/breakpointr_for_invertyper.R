	...
	if (numCPU > 1) {
		...
		time <- proc.time() - ptm; message(" ",round(time[3],2),"s")
			...
			for (index in c(1:length(grangeslist))) {
				temp[[index]] <- runBreakpointr_for_InvertypeR(granges = grangeslist[[index]], paired_reads=FALSE ,plotspath=plotspath,windowsize=windowsize, chromosomes=chromosomes,
